...
export default async function CategoryPage({ params }: { params: Promise<{ type: string }> }) {
    ...
    const normalizedHandle = type === "pre-rolls" ? "pre-roll" : type;
        ...
        <div className="min-h-screen bg-background">
            <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <div className="flex flex-col pb-8 sm:flex-row sm:items-end sm:justify-between text-left">
                    <div>
                        ...
                        <h1 className="text-5xl font-bold tracking-tighter text-foreground sm:text-7xl">
                            Shop <span className="text-muted-foreground">{title}.</span>
                        </h1>
                    </div>
                </div>
                ...
                {filteredProducts.length === 0 && (
                    <div className="mt-32 flex flex-col items-center justify-center space-y-4 text-center">
                        ...
                        <p className="max-w-md text-muted-foreground">
                            We're currently updating our {type} inventory. Check back soon for the latest drops!
                        </p>
                ...
                {collectionData?.descriptionHtml && (
                    <section className="mx-auto mt-4 mb-16 border-t border-border/10">
